...
def pr_denormalize(pr: xr.DataArray) -> xr.DataArray:
    eps = 10**-3
    exponent = pr*(np.log(max + eps) - np.log(eps)) + np.log(eps)
    pr = np.exp(exponent) - eps
    return pr

def pr_normalize(pr: xr.DataArray) -> xr.DataArray:
    eps = 10**-3
...
def denormalize(ds: xr.DataArray, transform) -> xr.DataArray:
    for varname in transform:
        ...
        if varname == "pr":
            ds["pr"] = pr_denormalize(ds["pr"], transform["pr"].min, transform["pr"].max)
        else:
